...
def last_timestamp(infile): # Expecting most of this to be command line input, so strings
    ...
    file_end = os.stat(infile).st_size
    ...
    next_chunk = 16 + int.from_bytes(packet_header[8:12], byteorder=endianness)
    ...
    # Packets
    while (in_ptr.tell()+next_chunk) < file_end:
        
        packet_header = in_ptr.read(next_chunk)
        #timestamp = packet_header[-16:-12]
        next_chunk = 16 + int.from_bytes(packet_header[-8:-4], byteorder=endianness)
