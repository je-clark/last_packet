...
def last_timestamp(infile): # Expecting most of this to be command line input, so strings
    ...
    while True:
        ...
        packet = in_ptr.read(packet_length + 16)
        if not packet:
        	break
        
        #timestamp = int.from_bytes(packet[-16:-12], byteorder=endianness)
        timestamp = packet[-16:-12]
        packet_length = int.from_bytes(packet[-8:-4], byteorder=endianness)
